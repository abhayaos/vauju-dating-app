...
// Separate component to handle auth context safely
function AuthConsumer({ children }) {
  try {
    const auth = useAuth();
    return children(auth);
  } catch (error) {
    // Return children with default values if auth context is not available
    return children({ token: null, logout: () => {} });
  }
}
    ...
    <AuthConsumer>
      {({ token, logout }) => {
        ...
        useEffect(() => {
          ...
          // Check immediately on mount, but only if we're not on login/register pages
